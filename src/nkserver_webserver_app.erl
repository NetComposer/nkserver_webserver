...
start(_Type, _Args) ->
    ...
    case nklib_config:load_env(?APP, Syntax) of
        {ok, _} ->
            {ok, Pid} = nkserver_webserver_sup:start_link(),
            {ok, Vsn} = application:get_key(nkserver, vsn),
            lager:info("NkSERVER REST v~s has started.", [Vsn]),
            nkserver_util:register_package_class(<<"Webserver">>, nkserver_webserver),
            {ok, Pid};
        {error, Error} ->
